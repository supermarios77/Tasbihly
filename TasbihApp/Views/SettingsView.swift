                                ...
                                if notificationManager.isNotificationsEnabled {
                                    HStack {
                                        ...
                                        DatePicker("", 
                                            ...
                                            .onChange(of: notificationManager.reminderTime) { newTime in
                                                HapticManager.shared.selectionChanged()
                                                // Schedule notification with new time
                                                notificationManager.scheduleNotification(newTime)
                                            }
                                    }
                                    .padding()
                                    .background(listRowBackground)
...
struct ThemePicker: View {
    ...
    private var safeThemeIndex: Int {
        min(max(selectedThemeIndex, 0), appThemes.count - 1)
    }
    
    private var safeThemeName: String {
        guard appThemes.indices.contains(safeThemeIndex) else {
            return appThemes.first?.name ?? "Default"
        }
        return appThemes[safeThemeIndex].name
    }
...
struct SettingsView_Previews: PreviewProvider {
    static var previews: some View {
        SettingsView(isSoundEnabled: .constant(true), target: .constant(500))
    }
}

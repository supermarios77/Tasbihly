...
struct TasbihView: View {
            ...
            ZStack {
                ...
                .alert(isPresented: $showCompletion) {
                    Alert(
                        ...
                        message: Text(
                            ...
                            Total: \(counter)
                            Sets: \(selectedDhikr.count > 0 ? counter / selectedDhikr.count : 0)
                            """
                        ),
                ...
                .alert(isPresented: $showResetConfirmation) {
                    Alert(
                        ...
                        message: Text("This will reset your counter to zero. This action cannot be undone."),
    ...
    private var counterButton: some View {
        ...
        .accessibilityHint("Tap to increase count. Total count is \(counter). Current set: \(selectedDhikr.count > 0 ? counter % selectedDhikr.count : 0) out of \(selectedDhikr.count)")
    }
    ...
    private func handleCounterTap() {
        ...
        // Check if set is complete (with safety check for division by zero)
        if selectedDhikr.count > 0 && counter % selectedDhikr.count == 0 {
    ...
    private struct ProgressRings: View {
        ...
        private var progress: CGFloat {
            guard selectedDhikr.count > 0 else { return 0.0 }
            return min(CGFloat(counter % selectedDhikr.count) / CGFloat(selectedDhikr.count), 1.0)
        }
    }
    ...
    private struct CounterDisplay: View {
        ...
        var body: some View {
            VStack(spacing: 4) {
                ...
                HStack(spacing: 0) {
                    Text("\(selectedDhikr.count > 0 ? counter % selectedDhikr.count : 0)")
                ...
                Text("Set \((selectedDhikr.count > 0 ? (counter / selectedDhikr.count) + 1 : 1))")
    ...
    var body: some View {
        VStack(spacing: 15) {
            ...
            VStack(spacing: 8) {
                ...
                StatItem(title: "Completed Sets", value: "\(selectedDhikr.count > 0 ? counter / selectedDhikr.count : 0)", theme: currentTheme)
                StatItem(title: "Current Set", value: "\((selectedDhikr.count > 0 ? counter % selectedDhikr.count : 0))/\(selectedDhikr.count)", theme: currentTheme)
            }
        }
    }
}
